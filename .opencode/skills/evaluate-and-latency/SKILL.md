...
- `eval_output/recall_summary.json` — overall `recall@k` (k=3).
- `eval_output/recall_at_3_details.json` — per-query records: `query`, `file_id`, `relevant_ids`, `retrieved_ids`, `retrieved_file_ids`, `recall`.

**Do not modify the scripts to change k.** The script hardcodes `k=3`. If another k is needed, import `recall_at_k` and call it with the desired `k` from a one-off `python -c` command.

Retrieve the results by reading both JSON files and report:

- Overall `recall@3` value.
- For k=3, list each query that scored `recall: 0` (missed the source chunk) — include the query text, expected file_id, and retrieved ids — and summarize any obvious pattern (e.g., wrong file, wrong chunk, empty retrieval).
...
Print a concise summary to the user: recall@3, missed-query count, and per-endpoint mean latency from each of the three sources (or note which source was unreachable). Flag any obvious discrepancy between sources.
...
Then read `eval_output/recall_summary.json` and `eval_output/recall_at_3_details.json`.