...
// Initial sample medical knowledge base
const initialSampleDocs: { doc: StoredDocument; chunks: string[] }[] = [
  {
    doc: {
      id: 'doc-who-hpv-2024',
      filename: 'WHO_HPV_Genotyping_Guidelines_2024.pdf',
      fileType: 'application/pdf',
      fileSize: 420500,
      uploadedAt: new Date(Date.now() - 3600000 * 24 * 3).toISOString(),
      status: 'indexed',
      chunksCount: 4,
      snippet: 'WHO technical recommendations on HPV DNA genotyping strategies across various clinical and regional screening settings.'
    },
    chunks: [
      'In countries with high follow-up capacity and robust diagnostic triage, HPV DNA testing with extended genotyping for HPV 16/18 is recommended as the primary screening method. Women testing positive for HPV 16 or 18 should be immediately referred for colposcopy.',
      'In resource-constrained settings or regions with low follow-up capacity, partial genotyping or standard high-risk HPV DNA testing with immediate screen-and-treat (thermal ablation or cryotherapy) is recommended to minimize loss to follow-up.',
      'HPV 16 and 18 account for approximately 70% of all cervical cancer cases worldwide. Genotyping allows rapid stratification of high-risk patients without requiring immediate biopsy for all HPV-positive cases.',
      'Screening intervals: HPV DNA testing is recommended every 5 to 10 years for women aged 30 to 49 years in the general population, and every 3 to 5 years for women living with HIV starting at age 25.'
    ]
  },
  {
    doc: {
      id: 'doc-iom-consensus-methods',
      filename: 'IOM_Clinical_Practice_Guidelines_Standards.pdf',
      fileType: 'application/pdf',
      fileSize: 680200,
      uploadedAt: new Date(Date.now() - 3600000 * 24 * 5).toISOString(),
      status: 'indexed',
      chunksCount: 3,
      snippet: 'Institute of Medicine (IOM) standards for developing trustworthy clinical guidelines and evaluating consensus methods.'
    },
    chunks: [
      'The main limitations of using informal consensus methods for developing clinical guidelines include susceptibility to dominant personalities, lack of systematic evidence synthesis, poor reproducibility, and vulnerability to unmanaged conflicts of interest.',
      'To determine which clinical topics should be addressed in guidelines, the Institute of Medicine (IOM) recommends evaluating disease burden, prevalence, variability in clinical practice, potential health outcome improvements, and economic impact.',
      'Formal consensus methods (such as modified Delphi techniques or nominal group techniques) provide structured, anonymous iteration and statistical aggregation, significantly outperforming informal round-table consensus.'
    ]
  },
  {
    doc: {
      id: 'doc-acip-immunization-review',
      filename: 'CDC_ACIP_Vaccine_Review_Procedures.pdf',
      fileType: 'application/pdf',
      fileSize: 310800,
      uploadedAt: new Date(Date.now() - 3600000 * 24 * 7).toISOString(),
      status: 'indexed',
      chunksCount: 2,
      snippet: 'Procedures used by the Advisory Committee on Immunization Practices (ACIP) for vaccine topic selection and evidence review.'
    },
    chunks: [
      'The Advisory Committee on Immunization Practices (ACIP) decides which vaccine topics to review and develop guidelines for based on: 1) FDA licensure status of new vaccines or expanded indications; 2) changing disease epidemiology; 3) new safety or efficacy signals; and 4) public health priority outbreaks.',
      'ACIP utilizes the GRADE (Grading of Recommendations Assessment, Development and Evaluation) framework and the Evidence to Recommendation (EtR) framework to systematically evaluate the balance of benefits and harms before drafting immunization recommendations.'
    ]
  }
];
...
initialSampleDocs.forEach(({ doc, chunks: chunkList }) => {
  documents.push(doc);
  chunkList.forEach((text, idx) => {
    chunks.push({
      id: `${doc.id}-chk-${idx + 1}`,
      fileId: doc.id,
      ...
      content: text,
      chunkIndex: idx,
      tokensEstimate: Math.ceil(text.split(/\s+/).length * 1.3)
    });
  });
});
...
const metricsState = {
  totalQueries: 142,
  localSearchCount: 114,
  webFallbackCount: 28,
  totalLatencyAccumMs: 142 * 410,
  uploadedFilesCount: initialSampleDocs.length,
