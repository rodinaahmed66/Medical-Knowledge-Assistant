...
const recommendedTopics = [
  {
    prefix: 'WHO Cervical Triage',
    question: 'What is the triage algorithm for high-risk HPV positive screening?'
  },
  {
    prefix: 'ACIP Immunization',
    question: 'What are the CDC recommendations for HPV vaccination schedule?'
  },
  {
    prefix: 'Statin Therapy',
    question: 'What are the primary prevention indications for high-intensity statin therapy?'
  },
  {
    prefix: 'Standard of Care',
    question: 'What are the IOM criteria for establishing clinical practice guidelines?'
  }
];
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 flex flex-col min-h-[calc(100vh-5rem)]">
      ...
      <div className="flex-1 space-y-6 pb-6">
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 bg-white border border-slate-200/90 rounded-2xl p-6 shadow-xs text-slate-700 space-y-4">
            ...
            <div className="space-y-2 text-sm pt-1">
              <div className="font-semibold text-slate-800 text-sm">Recommended Questions:</div>
              <div className="space-y-1.5 text-sm leading-relaxed">
                {recommendedTopics.map((item, idx) => (
                  <div key={idx} className="cursor-pointer hover:text-blue-600 transition-colors" onClick={() => handleSend(`${item.prefix}: ${item.question}`)}>
                    <span className="font-bold text-slate-900">{item.prefix}: </span>
